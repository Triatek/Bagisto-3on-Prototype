...
<footer class="mt-9 bg-lightOrange max-sm:mt-10">
    <div class="flex justify-between gap-x-6 gap-y-8 p-[60px] max-1060:flex-col-reverse max-md:gap-5 max-md:p-8 max-sm:px-4 max-sm:py-5">
        ...
        <div class="flex items-center justify-center max-1060:hidden">
            <a href="{{ route('shop.home.index') }}" aria-label="Home">
                <img
                    src="{{ core()->getCurrentChannel()->logo_url ?? bagisto_asset('images/logo.svg') }}"
                    alt="{{ core()->getCurrentChannel()->name }}"
                    class="h-auto max-h-[80px] w-auto max-w-[250px]"
                />
            </a>
        </div>
    ...
    <div class="flex justify-center bg-[#F1EADF] px-[60px] py-3.5 max-sm:px-5">
    
        {!! view_render_event('bagisto.shop.layout.footer.footer_text.before') !!}

        <p class="text-sm text-zinc-600 text-center">
            &copy; Copyright {{ date('Y') }}, 3on Sportsware Store by Triatek. All rights reserved.
        </p>
